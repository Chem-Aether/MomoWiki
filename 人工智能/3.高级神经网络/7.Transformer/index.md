...
现在需要设计一种方法，让序列中字符在被计算后只包含它之前地上下文信息，而不让它提前看到后文，即可使用如下的掩码操作：
...
计算相似度矩阵$Q_{dec} \times K_{enc}^T$，在进行$softmax(Q_{dec}K_{enc}^T)$归一化处理得到：
...
1. 训练时，每条数据将以 “我爱你” -> "I love you"对应同时输入编码器和解码器，序列`<我爱你>`在编码器中生成带上下文语义的向量；
